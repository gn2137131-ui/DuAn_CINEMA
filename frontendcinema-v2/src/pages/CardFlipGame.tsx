...
const CardFlipGame: React.FC = () => {
  ...
  const { cost, incrementPlayCount } = useGameCost('card-flip');
      ...
      <div className="flex-grow flex flex-col items-center justify-center py-10 relative overflow-hidden">
        ...
        {cost > 0 && (
          <div className="z-10 mb-6 bg-red-900/40 border border-red-500/30 py-2 px-4 rounded-xl text-red-200 font-bold text-center">
            ⚠️ Lượt lật này tốn: <span className="text-white text-xl">{cost} CineCoins</span>
          </div>
        )}
        ...
        {resultMsg && (
          <div className="flex flex-col items-center z-10 animate-fade-in-up">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={() => { setOpenedCard(null); setPointsWon(null); setResultMsg(null); }} className="px-6 py-3 bg-white text-slate-900 rounded-full font-bold uppercase hover:bg-slate-200 shadow-lg">
                CHƠI TIẾP
              </button>
            </div>
          </div>
