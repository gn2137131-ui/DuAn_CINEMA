...
export function Showtimes() {
        ...
        return (
            <div className="card overflow-hidden">
                <div className="card-head">
                    <span>Timeline Lịch chiếu (08:00 - 24:00)</span>
                    <div style={{ display:'flex', gap:'12px', fontSize:'0.75rem' }}>
                        <span style={{ display:'flex', alignItems:'center', gap:'4px' }}><span style={{ width:'10px', height:'10px', borderRadius:'2px', background:'var(--accent-success)', display:'inline-block' }}></span> Trống</span>
                        <span style={{ display:'flex', alignItems:'center', gap:'4px' }}><span style={{ width:'10px', height:'10px', borderRadius:'2px', background:'var(--accent-warning)', display:'inline-block' }}></span> Đang hot</span>
                        <span style={{ display:'flex', alignItems:'center', gap:'4px' }}><span style={{ width:'10px', height:'10px', borderRadius:'2px', background:'var(--accent-danger)', display:'inline-block' }}></span> Gần đầy</span>
                        <span style={{ display:'flex', alignItems:'center', gap:'4px' }}><span style={{ width:'10px', height:'10px', borderRadius:'2px', background:'var(--accent-danger)', border:'1px solid var(--accent-danger)', display:'inline-block' }}></span> Trùng lịch</span>
                    </div>
                </div>
                <div className="overflow-x-auto">
                    <div style={{ minWidth: '900px' }}>
                        <div style={{ display:'flex', borderBottom:'1px solid var(--border)', background:'var(--bg-elevated)', position:'sticky', top:0, zIndex:10, color:'var(--text-secondary)', fontSize:'0.75rem', fontWeight:'700' }}>
                            <div style={{ width:'144px', flexShrink:0, padding:'12px', borderRight:'1px solid var(--border)' }}>Phòng chiếu</div>
                            <div style={{ flex:1, position:'relative', height:'40px' }}>
                                {[8,10,12,14,16,18,20,22,24].map(hour => (
                                    <div key={hour} style={{ position:'absolute', top:0, bottom:0, borderLeft:'1px solid var(--border)', paddingLeft:'4px', paddingTop:'8px', left:`${((hour*60 - TIMELINE_START)/TIMELINE_DURATION)*100}%` }}>
                                        {hour}:00
                                    </div>
                                ))}
                            </div>
                        ...
                        {Object.values(roomGroups).map(rg => (
                            <div key={rg.room.id} style={{ display:'flex', borderBottom:'1px solid var(--border)', transition:'var(--transition)', position:'relative', minHeight:'70px' }}>
                                <div style={{ width:'144px', flexShrink:0, padding:'12px', borderRight:'1px solid var(--border)', fontWeight:'700', fontSize:'0.875rem', color:'var(--text-primary)', display:'flex', flexDirection:'column', justifyContent:'center' }}>
                                    {rg.room.name}
                                    <span style={{ fontSize:'0.65rem', color:'var(--text-muted)', fontWeight:'400' }}>{rg.room.type}</span>
                                </div>
                                <div style={{ flex:1, position:'relative', background:'var(--bg-input)' }}>
                                    {rg.slots.map((s, idx) => (
                                        <div 
                                            ...
                                            style={{ position:'absolute', top:'8px', bottom:'8px', borderRadius:'var(--radius-md)', border:'1px solid', fontSize:'0.75rem', overflow:'hidden', cursor:'pointer', left:`${s.left}%`, width:`${s.width}%`, background: s.conflict ? 'rgba(225,29,72,0.15)' : (s.occupancy >= 80 ? 'rgba(239,68,68,0.15)' : s.occupancy >= 50 ? 'rgba(245,158,11,0.15)' : 'rgba(34,197,94,0.15)'), borderColor: s.conflict ? 'var(--accent-danger)' : (s.occupancy >= 80 ? 'rgba(239,68,68,0.5)' : s.occupancy >= 50 ? 'rgba(245,158,11,0.5)' : 'rgba(34,197,94,0.5)'), color: s.conflict ? 'var(--accent-danger)' : (s.occupancy >= 80 ? 'var(--accent-danger)' : s.occupancy >= 50 ? 'var(--accent-warning)' : 'var(--accent-success)') }}
                                            className="hover:shadow-md transition-all"
                                        >
                                            <div style={{ padding:'4px 8px', height:'100%', display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
                                                <div style={{ fontWeight:'700', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{s.movieTitle}</div>
                                                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', opacity:0.8, fontWeight:'500', fontSize:'0.65rem' }}>
    ...
    return (
        <div className="admin-page">
            ...
            <div className="card">
                <div className="card-head">
                    <span><Monitor size={16} /> Giám sát trạng thái phòng</span>
                </div>
                        ...
                        return (
                            <div key={room.id} className="card card-pad">
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                                    ...
                                    <span className="badge badge-gray">{room.type}</span>
                                </div>
                                <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                        Trạng thái: 
                                        {hasShowtimeToday ? (
                                            <span className="badge badge-yellow">Có lịch chiếu</span>
                                        ) : (
