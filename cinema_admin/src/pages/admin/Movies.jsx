...
export function Movies() {
    ...
    const handleToggleHot = async (movie) => {
        try {
            await axiosClient.put(`/movies/${movie.id}/hot`);
            fetchMovies();
        } catch (err) {
                        ...
                        return (
                            <div key={movie.id} className="card" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                                ...
                                <div style={{ position: 'relative', aspectRatio: '3/4', background: 'var(--bg-elevated)', overflow: 'hidden' }}>
                                    ...
                                    {/* HOVER OVERLAY */}
                                    <div className="hover-overlay" style={{ display: 'flex', gap: '6px', padding: '16px', flexWrap: 'nowrap', justifyContent: 'center', alignItems: 'center' }}>
                                        {movie.trailerUrl && (
                                            <button className="action-btn play" onClick={() => handlePlayTrailer(movie.trailerUrl)} title="Xem Trailer">
                                                <Play size={16} fill="currentColor" />
                                            </button>
                                        )}
                                        <button className={`action-btn hot ${movie.is_hot ? 'active' : ''}`} onClick={() => handleToggleHot(movie)} title={movie.is_hot ? "Bỏ ghim" : "Ghim phim HOT"}>
                                            <Star size={16} fill={movie.is_hot ? "currentColor" : "none"} />
                                        </button>
                                        <button className="action-btn schedule" onClick={() => handleQuickAddShowtime(movie)} title="Lên lịch chiếu">
                                            <CalendarClock size={16} />
                                        </button>
                                        <button className="action-btn view" onClick={() => handleViewDetail(movie)} title="Chi tiết">
                                            <Eye size={16} />
                                        </button>
                                        <button className="action-btn edit" onClick={() => handleEdit(movie)} title="Sửa">
                                            <Edit2 size={16} />
                                        </button>
                                        <button className="action-btn delete" onClick={() => handleDelete(movie.id)} title="Xóa">
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
                                </div>
