    ...
    return (
      <div className="admin-page" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>Đang tải dữ liệu dashboard...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="admin-page">
      ...
      <motion.div className="grid-4" style={{ marginBottom: '24px' }} variants={containerVariants} initial="hidden" animate="show">
        {stats.map((stat, i) => {
          ...
          return (
            <motion.div key={i} className="stat-card" variants={itemVariants} whileHover={{ scale: 1.03, y: -5 }} style={{ cursor: 'pointer', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', border: `1px solid ${stat.bg}` }}>
              <div>
                <p className="stat-label">{stat.label}</p>
                <h3 className="stat-value" style={{ color: stat.accent, textShadow: `0 0 10px ${stat.bg}` }}>{stat.value}</h3>
                <p className="stat-change" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  {stat.up
                    ? <ArrowUpRight size={13} style={{ color: '#22c55e' }} />
                    : <ArrowDownRight size={13} style={{ color: '#ef4444' }} />}
                  <span style={{ color: stat.up ? '#22c55e' : '#ef4444' }}>{stat.change}</span>
      ...
      {/* CHARTS ROW */}
      <motion.div className="grid-3" style={{ marginBottom: '24px', display: 'grid', gridTemplateColumns: '1.2fr 0.8fr 1fr', gap: '24px' }} variants={containerVariants} initial="hidden" animate="show">
        {/* Revenue Area Chart */}
              ...
              {(dashboardData.recentTransactions || []).length === 0 ? (
                ...
                return (
                  <tr key={b.id}>
                    ...
                    <td style={{ fontWeight: '600', color: 'var(--text-primary)' }}>{b.movie}</td>
