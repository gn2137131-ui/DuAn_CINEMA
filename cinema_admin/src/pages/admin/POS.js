...
export default function POS() {
  ...
  const fetchConcessions = async () => {
    try {
      const res = await axiosClient.get('/snacks/snacks');
      setConcessions(res.data.map(c => ({ ...c, quantity: 0 })));
    } catch (err) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      const bookingFoods = concessions
  ...
  const renderStep2 = () => {
    return (
      <div className="flex flex-col min-h-full">
         <h2 className="text-2xl font-black mb-6 text-[var(--text-primary)]">Chọn ghế — {selectedMovie?.title} <span className="text-[var(--text-muted)] font-medium mx-1">•</span> {selectedShowtime?.startTime}</h2>
         
         <div className="bg-[var(--bg-surface)] rounded-2xl border border-[var(--border)] p-8 flex flex-col items-center shadow-lg relative">
            {loadingSeats ? (
               ...
               <>
                  ...
                  <div className="w-full max-w-lg mb-12 relative">
                     <div className="h-3 bg-gradient-to-r from-[var(--bg-surface)] via-[var(--accent-info)] to-[var(--bg-surface)] opacity-50 w-full rounded-t-full shadow-[0_-5px_15px_rgba(14,165,233,0.3)]"></div>
                     <p className="text-center text-[var(--accent-info)] opacity-70 text-[0.75rem] font-black uppercase tracking-[0.3em] mt-3">Màn hình</p>
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                     {Object.keys(groupedSeats).sort().map(row => (
                        <div key={row} className="flex items-center gap-4">
                           <div className="w-6 text-center font-bold text-[var(--text-muted)] text-sm">{row}</div>
                           <div className="flex gap-2.5">
                              {groupedSeats[row].sort((a,b)=>a.number-b.number).map(seat => {
                                  let seatClasses = "h-10 rounded-lg flex items-center justify-center text-[0.85rem] font-bold transition-all ";
                                  seatClasses += seat.type === 'couple' ? 'w-[84px] ' : 'w-10 ';
                  ...
                  {/* Legend */}
                  <div className="mt-12 flex gap-6 bg-[var(--bg-elevated)] px-6 py-3 rounded-full border border-[var(--border)]">
                     <div className="flex items-center gap-2"><div className="w-5 h-5 bg-[var(--bg-elevated)] border border-[var(--border)] rounded"></div><span className="text-xs text-[var(--text-secondary)] font-bold uppercase tracking-wider">Ghế thường</span></div>
                     <div className="flex items-center gap-2"><div className="w-5 h-5 bg-[rgba(245,158,11,0.1)] border border-[rgba(245,158,11,0.3)] rounded"></div><span className="text-xs text-[var(--text-secondary)] font-bold uppercase tracking-wider">VIP</span></div>
                     <div className="flex items-center gap-2"><div className="w-5 h-5 bg-[var(--accent-warning)] border border-[var(--accent-warning)] rounded"></div><span className="text-xs text-[var(--text-secondary)] font-bold uppercase tracking-wider">Đang chọn</span></div>
                     <div className="flex items-center gap-2"><div className="w-5 h-5 bg-[var(--bg-elevated)] border border-[var(--border)] rounded opacity-50"></div><span className="text-xs text-[var(--text-muted)] font-bold uppercase tracking-wider">Đã bán</span></div>
                  </div>
               </>
            )}
         </div>
         ...
         <div className="mt-6 flex justify-between">
            <button onClick={() => setStep(1)} className="px-6 py-3.5 rounded-xl border border-[var(--border)] bg-[var(--bg-elevated)] font-bold cursor-pointer flex items-center gap-2 text-[var(--text-primary)] hover:bg-[var(--bg-surface)] transition-colors"><ChevronLeft size={18}/> Quay lại</button>
            <button onClick={() => { if(selectedSeats.length===0) toast.error('Vui lòng chọn ít nhất 1 ghế'); else setStep(3); }} className="px-8 py-3.5 rounded-xl border-none bg-gradient-to-r from-orange-500 to-red-500 text-white font-black cursor-pointer flex items-center gap-2 shadow-[var(--shadow-glow-orange)] hover:from-orange-400 hover:to-red-400 transition-colors hover:-translate-y-0.5 transform">Tiếp theo <ChevronRight size={18}/></button>
         </div>
      </div>
    );
  };
  ...
  const renderStep3 = () => (
    <div className="flex flex-col h-full">
       <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-black text-[var(--text-primary)] m-0">Combo & Đồ ăn</h2>
          <span className="text-[var(--text-muted)] text-sm font-medium">Tùy chọn — có thể bỏ qua</span>
       </div>

       <div className="flex-1 grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-5 content-start">
          {concessions.map(c => (
             <div key={c.id} className={`bg-[var(--bg-surface)] rounded-2xl border p-6 flex flex-col transition-all shadow-lg ${c.quantity > 0 ? 'border-[var(--accent-warning)] bg-[rgba(245,158,11,0.05)]' : 'border-[var(--border)]'}`}>
                <Popcorn size={32} className="text-[var(--accent-warning)] mb-4" />
                <h3 className="text-lg font-black text-[var(--text-primary)] m-0 mb-1.5">{c.name}</h3>
                <p className="text-sm text-[var(--text-secondary)] m-0 mb-6 flex-1 font-medium">{c.description || 'Bắp và nước giải khát'}</p>
                <div className="flex justify-between items-center">
                   <span className="font-black text-lg text-[var(--text-primary)]">{c.price.toLocaleString()}đ</span>
                   {c.quantity === 0 ? (
                      <button onClick={() => handleConcessionChange(c.id, 1)} className="w-10 h-10 rounded-full bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--text-primary)] cursor-pointer flex items-center justify-center hover:bg-[var(--accent-warning)] hover:text-white hover:border-[var(--accent-warning)] transition-colors"><Check size={20} className="hidden" />+</button>
                   ) : (
                      <div className="flex items-center gap-2.5 bg-[var(--bg-elevated)] rounded-full p-1 border border-[var(--border)]">
                         <button onClick={() => handleConcessionChange(c.id, -1)} className="w-8 h-8 rounded-full bg-[var(--bg-surface)] border border-[var(--border)] cursor-pointer flex items-center justify-center text-[var(--text-primary)] hover:bg-slate-700 transition-colors font-bold text-lg leading-none pb-1">-</button>
                         <span className="font-bold w-6 text-center text-[var(--text-primary)]">{c.quantity}</span>
                         <button onClick={() => handleConcessionChange(c.id, 1)} className="w-8 h-8 rounded-full bg-[var(--accent-warning)] text-white border-none cursor-pointer flex items-center justify-center shadow-[var(--shadow-glow-orange)] font-bold text-lg leading-none pb-0.5">+</button>
                      </div>
                   )}
                </div>
             </div>
       ...
       <div className="mt-6 flex justify-between">
          <button onClick={() => setStep(2)} className="px-6 py-3.5 rounded-xl border border-[var(--border)] bg-[var(--bg-elevated)] font-bold cursor-pointer flex items-center gap-2 text-[var(--text-primary)] hover:bg-[var(--bg-surface)] transition-colors"><ChevronLeft size={18}/> Quay lại</button>
          <button onClick={() => setStep(4)} className="px-8 py-3.5 rounded-xl border-none bg-gradient-to-r from-orange-500 to-red-500 text-white font-black cursor-pointer flex items-center gap-2 shadow-[var(--shadow-glow-orange)] hover:from-orange-400 hover:to-red-400 transition-colors hover:-translate-y-0.5 transform">Tiếp theo <ChevronRight size={18}/></button>
       </div>
    </div>
  );
    ...
    <div className="flex h-[calc(100vh-64px)] bg-[var(--bg-base)] -m-6 font-sans text-[var(--text-primary)]">
       ...
       <div className="w-[360px] min-w-[360px] bg-[var(--bg-sidebar)] border-r border-[var(--border)] flex flex-col z-10 shadow-lg">
          ...
          <div className="p-6 border-t border-[var(--border)] bg-[var(--bg-surface)]">
             ...
             <div className="flex justify-between mb-3 text-[0.9rem] text-[var(--text-secondary)] font-medium">
                <span>Combo ({concessions.reduce((s,c)=>s+c.quantity, 0)})</span>
