    ...
    <div className="admin-page">
      ...
      <div className="card" style={{ marginBottom: '16px' }}>
        <div className="filter-bar">
          ...
          <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className="form-control" style={{ width: '190px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                          {user.avatar || user.avatarUrl ? (
                            <img src={user.avatar || user.avatarUrl} alt={displayName}
                              style={{ width: '38px', height: '38px', borderRadius: '50%', objectFit: 'cover', border: '2px solid var(--border)' }} />
                          ) : (
                            <div className="avatar" style={{ background: avatarGrad }}>{initials}</div>
                          )}
                          <span style={{ fontWeight: '600' }}>{user.fullName || 'Chưa cập nhật'}</span>
                        </div>
                      </td>
                      <td>
                        <span style={{ fontFamily: 'monospace', fontSize: '0.8rem', fontWeight: '700', color: 'var(--accent-primary)', background: 'rgba(249,115,22,0.08)', padding: '3px 8px', borderRadius: '6px' }}>
                          @{user.username}
                        </span>
                      </td>
