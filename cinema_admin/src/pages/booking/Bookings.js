...
export function Bookings() {
  ...
  return (
    <div className="admin-page">
      ...
      {/* STATS */}
      <motion.div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '20px', marginBottom: '24px' }} variants={containerVariants} initial="hidden" animate="show">
        {[
          { label: 'Đặt vé hôm nay', value: isLoading ? '...' : bookingsToday.length, accent: '#6366f1', bg: 'rgba(99,102,241,0.1)' },
          { label: 'Doanh thu hôm nay', value: isLoading ? '...' : revenueToday.toLocaleString('vi-VN') + 'đ', accent: '#22c55e', bg: 'rgba(34,197,94,0.1)' },
          { label: 'Đơn chờ xử lý', value: isLoading ? '...' : pendingCount, accent: '#f59e0b', bg: 'rgba(245,158,11,0.1)' },
        ].map((s, i) => (
          <motion.div key={i} className="stat-card" variants={itemVariants} whileHover={{ scale: 1.03, y: -5, boxShadow: `0 10px 30px ${s.bg}` }} style={{ border: `1px solid ${s.bg}`, background: `linear-gradient(135deg, ${s.bg} 0%, transparent 100%)`, cursor: 'pointer' }}>
            <div>
              <p className="stat-label" style={{ fontWeight: '600' }}>{s.label}</p>
              <h3 className="stat-value" style={{ color: s.accent, fontSize: '1.75rem', textShadow: `0 0 15px ${s.bg}` }}>{s.value}</h3>
            </div>
          </motion.div>
        ))}
      </motion.div>
      ...
      {isLoading ? (
        <div style={{ textAlign: 'center', padding: '60px' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: '12px', fontSize: '0.875rem' }}>Đang tải...</p>
        </div>
      ) : (
            ...
            <table className="table">
              ...
              <motion.tbody variants={containerVariants} initial="hidden" animate="show">
                {filteredBookings.length === 0 ? (
                  ...
                  return (
                    <motion.tr key={booking.id} variants={itemVariants} whileHover={{ backgroundColor: 'rgba(99,102,241,0.03)' }}>
                      <td>
                      ...
                      <td style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>
                        ...
                        <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>{roomName}</span>
                      </td>
                      <td style={{ fontSize: '0.82rem' }}>{seatsList.join(', ')}</td>
